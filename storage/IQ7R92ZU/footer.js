...
$(document).ready(function () {
    $('div[data-article-abstract-to-be-fetched-wrapper] > div[data-article-abstract-get] a').on('click', function () {
        var wrapperDiv = $(this).closest('div[data-article-abstract-to-be-fetched-wrapper]');
        var articleId = $(wrapperDiv).data('articleid-for-abstract');
...
$(document).ready(function () {
    $('div[data-book-abstract-to-be-fetched-wrapper] > div[data-book-abstract-get] a').on('click', function () {
        var wrapperDiv = $(this).closest('div[data-book-abstract-to-be-fetched-wrapper]');
...
(function (ns, $) {
    ...
    var _transitionLength = 200;

    function changeDisplayOfNodes($allNodes, $nodesToShow) {
    ...
    function transitionNodes($nodesToShow) {
        return new Promise(function (resolve, reject) {
            setTimeout(function () {
    ...
    function changeLevelOfNodesDisplayed($currentNode, newRootNodeId, $additionalNodes) {
        var $context = $currentNode.closest('.js-tree-browse-wrap');
        // Set to show only those specified
        var $nodesToShow = $context.find('.js-tree-node-item[data-parent-id="' + newRootNodeId + '"]');
        ...
        return new Promise(function (resolve, reject) {
            changeDisplayOfNodes($allNodes, $nodesToShow).then(function () {
                return transitionNodes($nodesToShow);
            }).then(function () {
                resolve();
            });
        });
    ...
    function goDeeper() {
        ...
        var $nodesGreaterThanMaxDepth = null;

        // If we're at the max level, show any nodes that are deeper.
        if (currentLevel === maxLevel) {
            $nodesGreaterThanMaxDepth = $currentNode.next('.js-children').find('.js-tree-node-item').filter(_nodeGreaterThanMaxLevel);
        }

        // Display the next level of nodes.
        changeLevelOfNodesDisplayed($currentNode, newRootNode, $nodesGreaterThanMaxDepth).then(function () {
            // You clicked on a node with an arrow. Add it to the breadcrumbs.
            var $breadcrumbs = $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-breadcrumb-link');
            $breadcrumbs.parents('.js-breadcrumbs').removeClass('hide');
            ...
            // Also change the browse link.
            $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-link-all').addClass('hide');
            var $narrowLink = $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-link-narrowed');
            $narrowLink.text(browseContentInLinkText + currentNodeLabel);
            $narrowLink.attr('href', $narrowLink.data('hrefTemplate').replace('{0}', newRootNode));
    ...
    function breadcrumbClick() {
        ...
        changeLevelOfNodesDisplayed($this, newId);

        // If you clicked the root breadcrumb, hide all breadcrumbs.
        if ($this.is('.js-root-breadcrumb')) {
            $this.parents('.js-breadcrumbs').addClass('hide');

            // Change the "Browse content in ..." link to "Browse all content"
            $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-link-all').removeClass('hide');
            $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-link-narrowed').addClass('hide');
        } else {
            var currentNodeLabel = $this.closest('.js-tree-browse-wrap')
                .find('.js-tree-node-item[data-id="' + newId + '"]')
                .find('.js-tree-node-link')
                .text();
            var $narrowLink = $this.closest('.js-tree-browse-wrap').find('.js-tree-browse-link-narrowed');
            $narrowLink.text(browseContentInLinkText + currentNodeLabel);
            $narrowLink.attr('href', $narrowLink.data('hrefTemplate').replace('{0}', newId));
            $narrowLink.removeClass('hide');
        }
    }
    ...
    ns.init = function () {
        ...
        $(document.body).on('click', '.js-tree-node-link, .js-tree-node-arrow', goDeeper);
...
(function (ns, $) {
    ...
    ns.registerAnswerLinkClick = function () {
        $('.answer-link').on('click', function () {
            $collapsableElements = $(this).nextUntil('.js-collapsableEnd');
            var displayElement = $collapsableElements.is(':hidden');
            if (displayElement) {
                $collapsableElements.removeClass('answer-hidden');
                $(this).text('Hide answer');
            }
            else {
                $collapsableElements.addClass('answer-hidden');
    ...
    ns.handleUnresolvedRelatedObjectLinks = function () {
        $(document).on('click', 'a[isbn]', function () {
            ...
            // don't bother if there's already an href to use
            if (!href) {
                // trim and normalise null
                isbn = isbn === null ? "" : isbn.trim();
                legacySectionId = legacySectionId === null ? "" : legacySectionId.trim();

                // Is there an ISBN we can use?
                if (isbn !== "") {
                    // call API
                    // e.g., https://academic.oupdev.silverchair.com/Book/ResolveSectionNestedBookLink?bypassOutputCache=true&isbn=9780190247782&legacySectionId=acprof-9780190247782-miscMatter-9-note-2
                    $.get('/Book/ResolveSectionNestedBookLink', {
                        isbn: isbn,
                        legacySectionId: legacySectionId
                    })
                    .done(function (res) {
                          // if the controller gives us a URL, use that
                          if (res && res.TargetUrl) {
                              redirectUrl = res.TargetUrl;
                          }
                          window.top.location.assign(redirectUrl); // borrowed from loginReturnBack()
                    })
                    .fail(function () {
                          // if it failed, log and go to 404 page
                          if (console && console.log) {
                              console.log("/Book/ResolveSectionNestedBookLink failed!");
                          }
...
$(function () {
    ...
    $(document).on('click', '.facetShowMore', function (event) {
        var $facetList = $(this).closest('.sf-facet-list, .sf-tax-facet-list').find('.sf-facet:gt(11):visible, .sf-tax-facet:gt(11):visible');
        if ($facetList.length) {
            $facetList.animate({ height: 'toggle' }, 400);
            $(this).text('Show more');
        }
        else {
            $(this).closest('.sf-facet-list, .sf-tax-facet-list').find('.sf-facet:gt(11), .sf-tax-facet:gt(11)').not(':visible').animate({ height: 'toggle' }, 400);
            $(this).text('Show less');
        }
        event.preventDefault();
    });
    ...
    function collapseFacets() {
        $('.collapsible-facet .sf-facet-list, .sf-tax-facet-list').each(function () {
            if ($(this).find('.sf-facet, .sf-tax-facet').length > 12) {
                $(this).children('.facetShowMoreLi').remove();
                $(this).find('.sf-facet:gt(11), .sf-tax-facet:gt(11)').hide();
...
(function (SolrSearch, $, undefined) {
    ...
    bodyVar.on('click', '.js-collection-checkbox', function () {
        ...
        if ($(this).is(":checked")) {
            parentContainer.find('.js-collection-checkbox').prop('checked', true);
        }
        else {
            parentContainer.find('.js-collection-checkbox').prop('checked', false);
        }
...
(function (ns, $) {
    ...
    ns.queryBuilderHandler = function () {
        ...
        var param = queryParams.find(function (el) {
            return el.startsWith("searchType");
        });
        if (!param) {
            queryParams.push("searchType=advanced");
        }
        param = queryParams.find(function (el) {
