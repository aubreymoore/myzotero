 ...
 * Date: 10/20/2011
 * 
 * Modified by Edward Minnett to add support for unlimited word count.
        ...
        return this.each(function() {
            ...
            obj.keyup(function() {
                ...
                if(text === "") {
                    ...
                    temp_transform = $.trim(text).replace(/(\r\n)+|\r+|\n+|\s+/g,' ');
                	wordcount = temp_transform.split(/\s/).length;
                }
                if(!options.limit) {
                    $("#counter-text").html( "Word Count: " + wordcount);
...
/*
 THIS PLUGIN HAS BEEN REPLACED BY CKEDITOR (TODO REMOVE).
 ...
 * + iframe overlay bugfix https://forum.jquery.com/topic/jquery-splitter-not-working-over-iframe
 * 
...
function Hilitor(id, tag)
{
  ...
  var colors = ["#ffd1a6", "#a0ffff", "#9f9", "#f99", "#f6f"];
  var wordColor = [];
  var colorIdx = 0;
  var matchRegex = "";
  ...
  {
  	var startWithQuotes = false;
  	var endWithQuotes = false;
  	if (input.indexOf('"') == 0){
    ...
    if (startWithQuotes && endWithQuotes){
  ...
  // start highlighting at target node
  this.applyAdditional = function(input)
  {
    if(input == undefined || !input) return;
    this.setRegex(input);
    this.hiliteWords(targetNode);
...
function HtmlTruncator() {
    ...
    this.truncate = function(str, maxLength) {
        ...
        var inTag = false; // parsing a tag
        var inTagName = false; // parsing a tag name
        var endingTag = false; // parsing an ending tag
        var count = 0; // chars/words added
        var chopped = false; // result has been chopped?
        var entityChars = 0; // number of chars in parsed entity

        var currentTag = ""; // will contain a tag name

        var openTags = new Array(); // lit of unclosed tags found in the string
        ...
        for (var i = 0; i < sz; i++)
        {
            ...
            else if (inTag)
            {
                ...
                else if (inTagName && (c == ' ' || c == '>'))
                {
                    ...
                    {
                        var index = openTags.indexOf(currentTag);
                        if (index != -1){
        ...
        if (openTags.length > 0)
        {
            ...
            var remainingToken = str.substring(i);

            for (var j = openTags.length - 1; j >= 0; j--)
            {
                var closingTag = "</" + openTags[j] + ">";
...
(function($) {
  ...
  $.fn.areYouSure = function(options) {
    ...
    var getValue = function($field) {
      ...
      if ($field.is('select')) { type = 'select'; }
    ...
    var checkForm = function(evt) {
      ...
      var isFieldDirty = function($field) {
        ...
        if (undefined === origValue) { return false; }
