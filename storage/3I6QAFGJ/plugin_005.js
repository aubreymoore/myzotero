	...
	CKEDITOR.plugins.add( 'clipboard', {
		...
		init: function( editor ) {
			...
			editor.on( 'paste', function( evt ) {
				...
				// 	var textRegx = new RegExp('(?:<|&lt;)\/?(?:script|embed|object|frameset|frame|table|td|iframe|meta|link|style)(?:.|\\n)*?(?:>|&gt;)','gi');
				// 	data = data.replace(textRegx, '');
				// }
				...
				if ( data.indexOf( 'Apple-' ) > -1 ) {
					...
					if ( evt.data.type != 'html' ) {
						data = data.replace( /<strong"[^>]*>([^<]*)<\/strong>/gi, function( all, spaces ) {
							// Replace tabs with 4 spaces like Fx does.
							return spaces.replace( /\t/g, '&nbsp;&nbsp; &nbsp;' );
						} );
					}
	...
	function initPasteClipboard( editor ) {
		...
		function getClipboardDataByPastebin( evt, callback ) {
			...
			var containerOffset = 0,
				offsetParent,
				win = doc.getWindow(),
				pastebinHeight = Math.max( 1, win.getViewPaneSize().height - 20 ),
				pastebinTop = ( win.getScrollPosition().y - containerOffset + 10 ),
				windowIsInIframe = false;
				
			if(win.$.parent != win.$){
				var prevWin = win.$;
				win.$ = win.$.parent;
				windowIsInIframe = true;
				pastebinTop = ( win.getScrollPosition().y - containerOffset + 10 ),
				pastebinHeight = Math.max( 1, win.getViewPaneSize().height - 20 - $(prevWin.frameElement).offset().top - 35);
				//The 35 is for the footer
			}
			//PENSOFT END
