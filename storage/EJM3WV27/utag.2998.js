//tealium universal tag - utag.2998 ut4.0.201607151749, Copyright 2016 Tealium.com Inc. All Rights Reserved.
...
window.pianoTag_execute=function(){if(!tp.isInitialized){tp.push(["init",function(){tp.experience.init();}]);}else{tp.experience.execute();}
}
...
if(!templateShown){window.pianoComplete();}}]);tp.push(["addHandler","checkoutClose",function(event){switch(event.state){case'close':window.pianoComplete();}}]);try{tp.push(['setCustomVariable','userAuthentication',window.site_vars.userAuthentication]);}catch(e){}
