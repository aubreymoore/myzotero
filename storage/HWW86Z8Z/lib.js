...
return result;}
$("#q, #authors, #journal, #article_type").on("change keyup",function(e){updateSearchLabelVisibilities()});$("#advanced-search").on("click",".search-plus",function(e){e.preventDefault();var currentRow=$(this).closest(".advanced-search-row");var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");if(currentRow.is(".advanced-search-row:last")){$("#advanced-search-button").appendTo(newRow.find(".columns:last").prev());}
currentRow.after(newRow);$(document).foundation('equalizer','reflow');});$("#advanced-search").on("click",".search-minus",function(e){e.preventDefault();var currentRow=$(this).closest(".advanced-search-row");if(currentRow.is(".advanced-search-row:last")){var previousRow=currentRow.prev(".advanced-search-row");$("#advanced-search-button").appendTo(previousRow.find(".columns:last").prev());}
currentRow.remove();$(document).foundation('equalizer','reflow');});$("#advanced-search").on("change",".connector",function(e){$(this).closest(".advanced-search-row").removeClass("and or").addClass($(this).val());});$("#advanced-search").on("click","#advanced-search-button",function(e){e.preventDefault();var queryString="(";$("#advanced-search").find(".advanced-search-row:not(:first)").each(function(e){queryString+=generateSearchFieldTerm($(this));});queryString+=")";if("()"!==queryString)
...
{$('#basic_search .search-input-label').show();$("#advanced-search").show();$(".search-container__advanced").show();$("#advanced-search-button").appendTo($("#advanced-search-button-hideout"));$(".advanced-search-row").not("#advanced-search-template").remove();$(".js-search-collapsed-button-container").children().each(function(){$(this).appendTo($(".js-search-expanded-button-container"));});$(".js-search-collapsed-link-container").children().each(function(){$(this).appendTo($(".js-search-expanded-link-container"));});$(".js-search-collapsed-button-container").hide();$(".js-search-collapsed-link-container").hide();if(''===advancedQuery)
{var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");$(".advanced-search-row:last").after(newRow);var newRow=$("#advanced-search-template").clone();newRow.removeAttr("id");$("#advanced-search-button").appendTo(newRow.find(".columns:last").prev());$(".advanced-search-row:last").after(newRow);}
else
...
newRow.find('.search-field').val(field);$("#advanced-search-button").appendTo(newRow.find(".columns:last"));$(".advanced-search-row:last").after(newRow);}}}
...
return validated;});});}).call(this);(function(){$(function(){var current_url,div_my_query_history,div_save_my_query,fill_options,journal_changed,select_option,showSaveQuery,update_sections,update_special_issues;var empty_section_option='<option value="">--</option>';var all_sections_option='<option value="">All Sections</option>';var empty_special_issue_option='<option value="">--</option>';var all_special_issues_option='<option value="">All Special Issues</option>';fill_options=function(node,items,empty_option,all_option){var data;data=$.map(items,function(val,key){return "<option value=\""+val[0]+"\">"+val[1]+"</option>";}).join('');return $(node).html(data!==''?all_option+data:empty_option);};select_option=function(options,value){return options.each(function(){if($(this).attr('value')===value){$(this).attr('selected','selected');}});};update_sections=function(journal_name){if(journal_name===''){$('#section').html(empty_section_option).trigger("chosen:updated");$('#special_issue').html(empty_special_issue_option).trigger("chosen:updated");return;}
return $.getJSON("/journal/"+journal_name+"/get/sections",function(data){var sectionId;fill_options('#section',data,empty_section_option,all_sections_option);sectionId=$('span[data-section-id]').attr('data-section-id');if(sectionId){select_option($('#section > option'),sectionId);$('span[data-section-id]').remove();}
...
return $.getJSON("/journal/"+journal_name+"/sections/"+section_id+"/get/special_issues",function(data){var specialIssueId;fill_options('#special_issue',data,empty_special_issue_option,all_special_issues_option);specialIssueId=$('span[data-special-issue-id]').attr('data-special-issue-id');if(specialIssueId){select_option($('#special_issue > option'),specialIssueId);$('span[data-special-issue-id]').remove();}
...
else{saveButton.val('saving');saveData={'url_desc':description,'url':current_url};$.post(saveButton.data('saveurl-url'),saveData,function(result){var delete_img=div_my_query_history.data('delete-img-url');if(result.success){$('#add-search-url').val('save');div_my_query_history.append('<div class="searchurl"><a class="savedurl-delete link--red" href="#" data-query-del="'+deleteurl+'?id='+parseInt(result.id)+'" style="vertical-align: top;"><i class="material-icons">remove_circle_outline</i></a><label class="search_refine_label"><a href="'+current_url+'" title="'+description+'">'+description.substring(0,25)+'</a></div>');$('#saveurl_description').val('');saveButton.closest('.reveal-modal').find('.close-reveal-modal').click();}
