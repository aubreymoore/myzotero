          ...
          if (xhr.status === 200 && resp) {
            ...
            if (imageUrl && detailsUrl && score) {
              var altLink = $('<a>')
                ...
                .attr('title', 'View Altmetric attention score details')
...
AOP.validateOrcidId = function (orcidId, customErrorElSelector, callback) {
  ...
  var output = false;
  $.ajax({
    url: '//pub.orcid.org/v1.1/search/orcid-bio/?q=orcid:' + orcidId,
...
(function ($) {
  ...
  $sideNavToggleDesktop.on('click', function () {
  ...
  //Mobile / tablet view
  //Main open close toggle side drawer
  $sideNavToggleMobile.on('click', function () {
    // $(this).toggleClass('open-drawer close');
