...
(function ($) {
    $.fn.showResults = function (oR, opt) {
        ...
        var p = { pg: '', n: 0, p: 0, pp: 0, np: 0, fp: 0, lp: 0, sp: 0, ep: 0, ls: 0, le: 0, tp: 0, is: 0, t: oR.length, r: oR, tpl: '' };
        ...
        return this.each(function () {
            ...
            if (o.field !== '') {
                if (o.searchKeyword == '') {
                    $key = '';
                    if (getParameterByName(o.queryParam).match("^" + o.field)) {
                        $key = getParameterByName(o.queryParam).substr(getParameterByName(o.queryParam).indexOf(":") + 1).replace(/\"/g, "");
                    }
                } else {
                    $key = o.searchKeyword;
                }
            ...
            self.doPag = function () {
                ...
                if (p.t > o.limit) {
                    ...
                    p.pp = ((o.start != 0) ? "<a href='#" + id + "-" + p.p + "' alt='" + p.p + "'><</a>" : "<span class='active'><</span>");
                    p.np = ((p.n < p.t && p.n != 0) ? "<a href='#" + id + "-" + p.n + "' alt='" + p.n + "'>></a>" : "<span class='active'>></span>");

                    p.fp = ((o.start != 0) ? "<a href='#" + id + "-" + 0 + "' alt='" + p.p + "'><<</a>" : "<span class='active'><<</span>");
                    p.lp = ((p.n < p.t && p.n != 0) ? "<a href='#" + id + "-" + ((p.ep - 1) * o.limit) + "' alt='" + p.n + "'>>></a>" : "<span class='active'>>></span>");

                    $pag.empty();
                    $pag.append(p.fp + p.pp + p.pg + p.np + p.lp);
...
function addPagingFromHash($elem) {
    var hashVal = window.location.hash.substr(1);
    var regex = /&pn=\d+/i;
    if ($($elem).attr('href').match(regex))
        $($elem).prop('href', $($elem).attr('href').replace(regex, "&pn=" + hashVal.substr(hashVal.indexOf("-") + 1)));
    else
        $($elem).prop('href', $($elem).attr('href') + "&pn=" + hashVal.substr(hashVal.indexOf("-") + 1));
};
